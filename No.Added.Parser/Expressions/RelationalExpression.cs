namespace No.Added.Parser.Expressions
{
    ...
    public class RelationalExpression : AdditiveExpression
    {
        public RelationalExpression(DefaultParser parser, TokenCode left, NodeType nodeType, TokenCode right) : base(parser, left, nodeType, right)
        {
        }
        ...
        {
            Node node = parser.TryRelational(code);
            if (node != null)
            {
                return node;
            }
