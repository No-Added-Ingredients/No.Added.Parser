namespace No.Added.Parser
{
    ...
    {
        protected Func<string, NodeType> tokenOfTree;

        protected Func<string, NodeType> tokenOfRoot;

     //   protected Func<char, NodeType> separateToken;
        ...
        public virtual IdentifierCode ParseIndentifierCode(ScriptReader stream)
        {
            ...
            var text = stream.SplitAt(stream.Position - 1);
            return new IdentifierCode() { Text = text, Type = this.ReservedWordCheck(text) };
        }

        public virtual bool IsTrue(string boolean)
        {
            return "true".Equals(boolean, System.StringComparison.Ordinal);
        }
        ...
        public ArgumentList TryArgumentList(TokenCode code)
        {
            ...
            var end = (code as SetStart).End;
            end.Next = Utils.CodeSplit;
            var result = new ArgumentList();
            code.ParseList(this, Utils.Comma, result, end);
            return result;
        }
        ...
        protected virtual NodeType ReservedWordCheck(string word)
        {
            switch(word)
            {
                case "false":
                ...
                default:
                    return NodeType.IdentifierName;
            }
        }

        public virtual Node ParseLiteral(IdentifierCode code)
        {
            // overload ParseLiteral for more Code types for flexibility.
            if (!code.Next.EndOfExpression)
            {
                throw this.Error("Literal is not end node.");
            }

            switch (code.Type)
            {
                case NodeType.Boolean:
                    return new BooleanLiteral(this, code);

                case NodeType.IdentifierName:
                    return new Identifier(this, code);

                default:
                    throw this.Error("Unexpected node type");
            }
        }
